<header class="align-items-center pb-3 mb-5 border-bottom text-end">
    <title>Hafizh Pratama | Software Engineer</title>
    <nav class="navbar navbar-expand-lg bg-light">
        <div class="container-fluid">
            <a class="navbar-brand" href="/"><img src="/images/logo.png"
                    class="d-inline-block align-top rounded" alt="logo" style="width: 50px;"></a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavDropdown"
                aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div style="flex-grow: unset;" class="collapse navbar-collapse" id="navbarNavDropdown">
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link color-black" href="/">Home</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link color-black" href="/blog">Blog</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link color-black" href="/certificates">Certificates</a>
                    </li>
                </ul>
            </div>
        </div>
    </nav>
</header>